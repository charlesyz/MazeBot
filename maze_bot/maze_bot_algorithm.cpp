...
// test for distance at myangle
float distance(int myangle) {
  	float Fdistance;
  	int pulsewidth;

 	 // turns servo to myangle
  	for (int i = 0; i <= 5; i++) {
    	// calculates how long to turn the servo
 	...
 	// ultrasound distance testing
  	digitalWrite(Trig, LOW);
  	delayMicroseconds(2);
  	digitalWrite(Trig, HIGH);   // sending echos
  	delayMicroseconds(10);
  	digitalWrite(Trig, LOW);

  	Fdistance = pulseIn(Echo, HIGH); // getting echo length in miliseconds
  	Fdistance = Fdistance / 58;      // calculating distance from ms
...
void loop() {
	...
	//=========================== maze algorithm ========================================
  	//if the car hits a junction, check surrounding pathways and do appropriate actions 
    	...
    	// checks 0 degrees(left) 90 degrees(forwards) and 180 degrees(right)
    	// the statement (dir + (i - 1) + 4) % 4 will get the direction (in NESW) that the sensor is pointing to.
    	for (int i = 0; i < 3; i++)
    		maze[posX][posY].connections[(dir + (i - 1) + 4) % 4] = distance(i * 90) > 20; 
		...
		// if no openings, turn around
			...
			left()
	        // turn for at least 1 second to get past the black line 
			delay(100);
